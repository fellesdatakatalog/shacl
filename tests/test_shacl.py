...
def test_validate_example():
    ...
    m.read_triples(
        str(REPO_PATH / "shacl" / "DCAT-AP-NO-shacl_shapes_2.00.ttl"), graph=SHAPE_GRAPH
    )
    report = m.validate(
        shape_graph=SHAPE_GRAPH, include_conforms=False, include_shape_graph=True
    )
    results_df = report.results()
    if results_df is not None:
        violations_df = results_df.filter(
            pl.col("result_severity").eq("<http://www.w3.org/ns/shacl#Violation>")
        )
        if violations_df.height > 0:
            print(
                "\n__Example file validation failures:__ {}\n".format(
                    violations_df.height
                ),
                file=f,
            )
...
def test_validate_concept_example():
    if "GITHUB_STEP_SUMMARY" in os.environ:
        f = open(os.environ["GITHUB_STEP_SUMMARY"], "a")
    else:
        f = None
    m = Mapping()
    m.read_triples(str(REPO_PATH / "examples" / "out.ttl"))
    m.read_triples(
        str(REPO_PATH / "shacl" / "SKOS-AP-NO-shacl_shapes_2.00.ttl"), graph=SHAPE_GRAPH
    )
    report = m.validate(
        shape_graph=SHAPE_GRAPH, include_conforms=False, include_shape_graph=True
    )
